...
func NewOrgRoutes(orgSvc *service.OrganizationService, cfg *config.Configuration, m *metrics.Client) *OrgRoutes {
	return &OrgRoutes{
		orgSvc:  orgSvc,
		config:  cfg,
		metrics: m,
...
func (or *OrgRoutes) RegisterRoutes(router chi.Router) {
	router.Route("/orgs", func(r chi.Router) {
		...
		r.Post("/", handlers.HandleCreateOrganization(or.orgSvc, or.metrics))
